...
MAX_PROFILE_SCROLLS_MIN = int(os.environ.get("X_REL_PROFILE_SCROLLS_MIN", str(MAX_PROFILE_SCROLLS)))
MAX_PROFILE_SCROLLS_MAX = int(os.environ.get("X_REL_PROFILE_SCROLLS_MAX", str(MAX_PROFILE_SCROLLS)))
MAX_EDGE_SCROLLS_MIN = int(os.environ.get("X_REL_EDGE_SCROLLS_MIN", str(MAX_EDGE_SCROLLS)))
...
def unavailable(body: str) -> bool:
    return bool(re.search(r"This account doesn't exist|Account suspended|These posts are protected", body, re.I))
...
async def scrape_connections(browser: ChromeMCPBrowser, url: str | None, edge_type: str, scroll_budget: int) -> list[dict]:
    if not url:
        return []
...
async def main() -> None:
    ...
    try:
        async with ChromeMCPBrowser(browser_url) as browser:
            ...
            for handle in HANDLES:
                ...
                try:
                    ...
                    if unavailable(payload["text"]):
                        result["profiles"].append({"handle": handle, "status": "unavailable"})
                        handle_result["status"] = "unavailable"
                        result["handle_results"].append(handle_result)
                        write_json("collect_relationships.json", result)
                        continue

                    profile_scroll_budget = choose_scroll_budget(MAX_PROFILE_SCROLLS_MIN, MAX_PROFILE_SCROLLS_MAX)
                    following_scroll_budget = choose_scroll_budget(MAX_EDGE_SCROLLS_MIN, MAX_EDGE_SCROLLS_MAX)
                    follower_scroll_budget = choose_scroll_budget(MAX_EDGE_SCROLLS_MIN, MAX_EDGE_SCROLLS_MAX)
                    handle_result["profile_scrolls_used"] = profile_scroll_budget
                    handle_result["following_scrolls_used"] = following_scroll_budget
                    handle_result["follower_scrolls_used"] = follower_scroll_budget

                    for _ in range(profile_scroll_budget):
                        await browser.scroll_page()
                    ...
                    if not profile.get("handle_match"):
                        ...
                        handle_result["resolved_handle"] = profile.get("resolved_handle")
                        result["handle_results"].append(handle_result)
                    ...
                    try:
                        following = [] if DIRECTION == "followers" else await scrape_connections(
                            browser, profile.get("following_url"), "following", following_scroll_budget
                        )
                    except Exception as error:
                        following = []
                        result["handle_errors"].append(
                            {"handle": handle, "phase": "following", "error": str(error)}
                        )

                    try:
                        followers = [] if DIRECTION == "following" else await scrape_connections(
                            browser, profile.get("followers_url"), "followers", follower_scroll_budget
                        )
                    except Exception as error:
                        followers = []
                        result["handle_errors"].append(
                            {"handle": handle, "phase": "followers", "error": str(error)}
                        )
            ...
            result["profile_count"] = sum(1 for p in result["profiles"] if p.get("status") == "ok")
            ...
            try:
                ...
                result["handle_errors"].append({"handle": "_session", "phase": "screenshot", "error": str(error)})
    except Exception as error:
        if result["profiles"] or result["edges"]:
            result["status"] = "partial"
            result.setdefault("handle_errors", []).append({"handle": "_session", "phase": "runner", "error": str(error)})
        else:
            result["status"] = "error"
